...
const MARKER_BEARING_PATH = "M 0 -2 L -7 5 L -10 2 L 0 -8 L 10 2 L 7 5";
...
export function Point({
  ...
  switch (route_type) {
    case RouteType.RAIL:
      typePath = RAIL_TYPE_PATH;
      break;
    case RouteType.BUS:
      typePath = BUS_TYPE_PATH;
      break;
    case RouteType.FERRY:
      typePath = FERRY_TYPE_PATH;
      break;
    case RouteType.CABLE_CAR:
      typePath = CABLE_CAR_TYPE_PATH;
      break;
    case RouteType.SCHOOL_BUS:
      typePath = SCHOOL_BUS_TYPE_PATH;
      break;
    default:
      break;
  }
  ...
  const bearingMarker = new google.maps.Marker({
    icon: {
  ...
  const typeMarker = new google.maps.Marker({
    icon: {
      ...
      fillOpacity: 1,
      path: MARKER_TYPE_PATH,
      scale: MARKER_TYPE_SCALE,
      strokeColor: "#ffffff",
      strokeWeight: 1,
    },
    position: { lat: latitude, lng: longitude },
  ...
  const typeIconMarker = new google.maps.Marker({
    icon: {
      anchor: new google.maps.Point(
        27.762 -
          ((12 * MARKER_TYPE_SCALE) / MARKER_TYPE_SCALE) * ICON_TYPE_SCALE,
        36.14 - ((12 * MARKER_TYPE_SCALE) / MARKER_TYPE_SCALE) * ICON_TYPE_SCALE
      ),
      fillColor: "#ffffff",
      fillOpacity: 1,
      path: typePath,
      scale: (1 / ICON_TYPE_SCALE) * MARKER_TYPE_SCALE,
      strokeWeight: 0,
    },
    position: { lat: latitude, lng: longitude },
    title: route_long_name,
  ...
  const markers = [marker, bearingMarker, typeMarker, typeIconMarker];
  markers.forEach((marker) => marker.addListener("click", onClick));
